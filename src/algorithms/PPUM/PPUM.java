...
public abstract class PPUM {
	...
	public String generateSensitive(String hui) {
		int iLen=hui.length();
		String temp=hui.substring(0,iLen-4);
	
		BufferedReader br = FileTool.getReader(hui);
		BufferedWriter bw = FileTool.getWriter(temp + "_si_"+sensitiveThreshold+".txt");
...
//							+ "-----------" + del);
	...
	public void mining(String src, String dst) {
		...
		File out_path = new File("spmf");
		if(!out_path.exists())
			out_path.mkdirs();
		
		int iLen=src.length();
		String temp=src.substring(0,iLen-4);
		
		String spmf_path=temp+"_spmf.txt";
				
		String huiPath = transDBformat2HUIMiner(src, spmf_path);
